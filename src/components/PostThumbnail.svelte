...
<article class="flex flex-row w-full h-40 p-3 my-4 border border-gray-300 shadow-xl grid grid-rows-5 rounded-md">
  <div class="pb-3 overflow-hidden grid grid-cols-4 gap-4 row-span-4">
  {#if post.imageId}
    <div class="overflow-hidden col-span-1">
      <img class="w-full h-full" src={imageSrc} alt={post.title} />
    </div>
  {/if}
